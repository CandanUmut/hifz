...
describe('word list as the tokenisation', () => {
  it('keeps a waqf mark with its word instead of treating it as a word', () => {
    // ۖ is written after the word with a space, but belongs to it. Splitting on
    // whitespace invented a token that is not a word.
    const segment = {
